...
Important: Plan only. Do not implement anything. Do not assume functionality is missing; confirm with code search first. Feel free to place shared utilities and components in `src/lib`. Treat `src/lib` as the project's standard library for shared utilities and components. Prefer consolidated, idiomatic implementations there over ad-hoc copies.
