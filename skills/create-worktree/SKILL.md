...
This stage is focused on environment setup before implementation. It creates or describes an isolated worktree and provides the launch command for the implementation session.
...
implementation launch prompt:
    ...
    Open a fresh Codex session in [path], then send:
    [$implement-plan]([path]/.agents/skills/implement-plan/SKILL.md) [relative plan path]
...
Next step: open a fresh Codex session in the worktree:

`[path]`

Then send this implementation prompt:

`[$implement-plan]([path]/.agents/skills/implement-plan/SKILL.md) thoughts/shared/plans/...`

The plan path must stay relative:
...
- The handoff must tell the user which directory to open as the new session workspace and the exact skill invocation to send from that workspace.
